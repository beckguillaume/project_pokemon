...
def start(players):
    for player in (players):
        ...
        while i < len(dic_joueur): #a adapter en fonction de la bdd
            if dic_joueur[i]["joueur"] == player[1]:
                msg="De retour, "+player[1]+" ? Bonne chance !"
                creation=False
            i+=1
        if creation==True:
            msg="Bienvenue, "+player[1]+". Bonne chance !"
            #bdd create add.joueur
        print(msg)
        pokemon_socket.send_message(player[0],msg)
...
class qui_commence():
    ...
    def choix_poke_depart(self, pokemaster): 
        ...
        if vitesse[0][2] < vitesse[1][2]:
            msg="C'est "+vitesse[1][0]+" qui commence"
            #print(tab_jeu[0][2], tab_jeu[1][2])
            joueur_1=vitesse[1]
            joueur_2=vitesse[0]
        elif vitesse[0][2] > vitesse[1][2]:
            #print("cacahouette")
            msg="C'est "+vitesse[0][0]+" qui commence"
            joueur_2=vitesse[1]
            joueur_1=vitesse[0]
        else:
...
while nbr_poke_2 > 0 and nbr_poke_1 > 0 and game_in == True:
    ...
    while again == "on" and nbr_poke_1 > 0 and game_in == True:
        ...
        msg="\033[1;33m" + "C'est au tour de " + joueur_1[0] + "\033[0m"
        print(msg)
        pokemon_socket.broadcast_message(joueurs_sockets, msg)
...
pokemon_socket.close_server_socket(joueurs_sockets,server_socket)
