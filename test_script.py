...
class joueur():
    ...
    def presentation_joueur(self, nom):
        ...
        if pseudo_ok == True:
            ...
            # Lancement de la fonction de création d'un joueur
            self.crea_joueur(nom)
        self.nom = nom
    ...
    def crea_joueur(self, name):
        # Choix aléatoir des trois pokemon de démarage
        pok_1 = random.randrange(0, 809)
...
class pokemon():
    ...
    def presentation_pokemon(self, id):
        self.id = id
        print("id", self.id, "nom", self.nom, "type", self.type, "status", self.status, "actif", self.actif, "faiblesse", self.faiblesse, "resistance", self.resistance,
              "hp", self.hp, "attaque", self.attaque, "defense", self.defense, "sp_attaque", self.sp_attaque, "sp_def", self.sp_def, "vitesse", self.vitesse)
...
class action():
    ...
    def change(self):
        ls = joueur.id_poke_joueur(joueur_1)
        for i in range(len(ls)):
            print(ls[i])
            # id=ls[i]
            # pokemon.presentation_pokemon(id)

        # print(joueur_1.id_poke_joueur())
        pass
...
# joueur_1=joueur(1).presentation_joueur
# print(joueur.id_poke_joueur(joueur_1))

# action(pok1, pok2).change()
